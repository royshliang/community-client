                            ...
                            <div class="col-12">
                                <div class="d-grid">
                                    <button class="btn btn-primary btn-lg" @click="login" :disabled="isLoading">
                                        <span v-if="!isLoading">Log in now</span>
    ...
    async function login() {
        ...
        if(fbToken) {
            try {
                ...
                await authStore.validate({email: email.value, token: fbToken});
                console.log("email:" + email.value)
                if(authStore.getUser != null) {
                    toast.success("login successful", { position: POSITION.TOP_CENTER, timeout: 1500} );
                }
            }
            catch(err) {
                Swal.fire({
